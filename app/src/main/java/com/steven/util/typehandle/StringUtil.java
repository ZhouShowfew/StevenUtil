package com.steven.util.typehandle;
...
public class StringUtil
{
	...
	public static String byteTostrGBK(byte[] data) throws UnsupportedEncodingException
	{
	 ...
	 */
	public static byte[] strToAscBytes(String des, int lenth) throws UnsupportedEncodingException
	{
		byte[] desBytes = new byte[lenth];
		byte[] tempDes = des.getBytes("GBK");
		if ( tempDes.length >= lenth )
		{
			return tempDes;
		}
		else
		{
			Arrays.fill(desBytes, (byte) 0x20);
			System.arraycopy(tempDes, 0, desBytes, 0, tempDes.length);
			return desBytes;
		}

	}

	public static byte[] strToAscBytesAddLeft(String des, int lenth) throws UnsupportedEncodingException
	{
		byte[] desBytes = new byte[lenth];
		byte[] tempDes = des.getBytes("GBK");
		if ( tempDes.length >= lenth )
		...
		{
			Arrays.fill(desBytes, (byte) 0x00);
			System.arraycopy(tempDes,0, desBytes,lenth-tempDes.length, tempDes.length);
			return desBytes;
		}
